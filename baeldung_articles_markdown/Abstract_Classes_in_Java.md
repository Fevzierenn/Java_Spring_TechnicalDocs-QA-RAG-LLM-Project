...
Let’s keep in mind that all these scenarios are good examples of full, inheritance-based adherence to the [Open/Closed principle](https://en.wikipedia.org/wiki/Open%E2%80%93closed_principle).

Moreover, since the use of abstract classes implicitly deals with base types and subtypes, we’re also taking advantage of [Polymorphism](/java-polymorphism).
...
**And[Java 8 adds another wrinkle with default methods](/java-static-default-methods), which can sometimes take the place of needing to create an abstract class altogether.**
...
For simplicity’s sake, the target file is located under the _src/main/resources/files_ folder. Hence, we used an application class loader for getting the path of the example file. Feel free to check out [our tutorial on class loaders in Java](/java-classloaders).

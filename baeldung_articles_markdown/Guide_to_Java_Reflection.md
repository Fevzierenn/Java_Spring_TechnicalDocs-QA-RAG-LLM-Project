...
This test shows us that we are able to get an array of _F_ _ield_ objects from our _person_ object, even if the reference to the object is a parent type of that object.
...
For instance, in many cases, we have a naming convention for database tables. We may choose to add consistency by prefixing our table names with _tbl__ so that a table with student data is called _tbl_student_data_.
...
The [_java.lang.reflect.Modifier_](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/reflect/Modifier.html) class offers static methods that analyze the returned _Integer_ for the presence or absence of a specific modifier.
...
With Java reflection, we can **inspect constructors** of any class and even **create class objects at runtime.** This is made possible by the [_java.lang.reflect.Constructor_](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/lang/reflect/Constructor.html) class.
